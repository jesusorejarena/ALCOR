										...
										<ul class='navbar-nav'>";
											...
											while (($cargo=$obj_car->extraer_dato())>0)
											{
												if ($_SESSION['cargo']==$cargo['cod_car'])
												{
													...
													{
														$obj_mod->cod_car=$permiso['cod_car'];
														$obj_mod->puntero=$obj_mod->listar_menu();

														while(($modulo=$obj_mod->extraer_dato())>0)
														{
														
															$obj_opc->cod_mod=$modulo['cod_mod'];
															$obj_opc->puntero=$obj_opc->listar_menu();
															...
															echo "
																...
																<div class='btn-group'>
																	<button type='button' class='btn btn-danger'>$modulo[nom_mod]</button>
															...
															while (($opcion=$obj_opc->extraer_dato())>0)
															{
																...
																echo "
																		<a class='dropdown-item' href='$opcion[url_opc]'>$opcion[acc_opc]</a>
																";
															}
