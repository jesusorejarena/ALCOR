...
$obj_ped->puntero = $obj_ped->getOrderByCodeUser();
$pedido = $obj_ped->extractData();

if (isset($_REQUEST['cod_ped']) || $pedido > 0) {
} else {
		...
		<div class="col-12 col-xl-4 p-md-2 mr-xl-5">
			...
			<div class="card rounded-lg shadow my-3 px-3 py-4">
				<form action="../../backend/controller/pedido.php" method="POST" class="was-validation" id="formulario" novalidate>
					...
					<?php
					if ($_REQUEST['info'] == 'true' && $pedido['est_ped'] == 'P') {
					?>
						<div class="px-4 pb-3 d-flex justify-content-between">
							<button type="submit" class="btn btn-danger" name="run" value="delete">Eliminar pedido</button>
							<button type="submit" class="btn btn-primary" name="run" value="updateStatusV">Confirmar</button>
						</div>
					<?php
					}
					?>
