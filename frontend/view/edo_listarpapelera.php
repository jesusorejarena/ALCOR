							...
							while (($proveedor = $obj_edo->extractData()) > 0) {
								echo "<form action='../../backend/controller/proveedor.php' method='POST'>
												<tr>
													...
													<td><button type='submit' class='$obj_edo->btn_restaurar' name='run' value='restore'><i class='icon ion-md-refresh'></i></button></td>
													<td><button type='submit' class='$obj_edo->btn_eliminar' name='run' value='delete'><i class='icon ion-md-trash'></i></button></td>
												</tr>
											</form>
										";
							}
